...
published:
created: 2026-05-09
description: "高强度用了半年 Claude Code，我的 ~/.claude/skills/ 目录下已经堆了几十个 Skill。"
tags:
  - "clippings"
---
中国人本来就聪明 *2026年3月2日 13:37*
...
1. 1\. 先调用飞书文档读取 Skill，拉取这个业务域的需求文档和技术设计文档，理解业务上下文
2. 2\. 然后调用代码仓库分析 Skill，定位到对应的代码目录，探索项目结构
3. 3\. 再调用接口链路追踪 Skill，找到核心接口的完整调用链路，画出时序图和流程图
4. 4\. 最后把所有信息整合，输出一份包含现状分析、链路梳理、改进建议的技术方案
...
1. 1\. **输入识别与获取** ：判断你给的是什么类型的输入，用合适的方式拿到完整内容
2. 2\. **深度分析** ：按多维框架分析输入的核心价值、适用场景、与现有能力的关系
3. 3\. **影响扫描** ：检查现有的 Skills、CLAUDE.md、rules，看哪些地方会被影响
4. 4\. **消化建议** ：给出具体的内化方案——新建 Skill、更新规则、还是扩展已有 Skill
5. 5\. **执行消化** ：用户确认后，执行具体的修改
